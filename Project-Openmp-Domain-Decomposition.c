...
void * solve_equation(void* rank_OpenMP)
{
	...
	#pragma omp private(j_end,j_start,i,j,myOp_ID)
	{
		...
		j_start = myOp_ID * ((nx-1)/omp_nthreads);
		j_end = j_start + ((nx-1)/omp_nthreads);
...
int main(int argc, char *argv[]) 
{
	...
	printf("OpenMp: Simulation of nx:%d,ny:%d,nt:%d,Lx:%lf,Ly:%lf,auto_save:%d,nthreads:%d, took %lf seconds, with %ld Kb\n", nx,ny,nt,Lx,Ly,auto_save,omp_nthreads, ttotal, MEMORY_USAGE);	
