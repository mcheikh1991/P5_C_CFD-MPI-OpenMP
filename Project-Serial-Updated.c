...
{
	char filename[20];
	snprintf(filename,20,"/homes/mcheikh/CIS_625/Project/Serial-Updated/output/boundary");
...
void *write_output(int time)
{
	...
	snprintf(filename1,20,"/homes/mcheikh/CIS_625/Project/Serial-Updated/output/u-%d",time);
	snprintf(filename2,20,"/homes/mcheikh/CIS_625/Project/Serial-Updated/output/v-%d",time);
...
int main(int argc, char *argv[]) 
{
	...
	*/
	...
	for (n=2;n<=nt;n+=2)
	{
		...
		solve_equation(); // solve bergers equation for u and v

		update_boundary(); // update the periodic boundary condition for u and v

		solve_equation2(); // solve bergers equation for un and vn

		update_boundary2(); // update the periodic boundary condition of un and vn
	...
	printf("Serial-Updated: Simulation of nx:%d,ny:%d,nt:%d,Lx:%lf,Ly:%lf,auto_save:%d, took %lf seconds, with %ld Kb\n", nx,ny,nt,Lx,Ly,auto_save, ttotal, MEMORY_USAGE);	
